...
#include <atrias_shared/RtAlloc.hpp>       // For realtime messages
#include <atrias_shared/RtCheck.hpp>       // We initialize RtCheck's event sending operation
#include <atrias_shared/RtMsgTypekits.hpp> // For registering RT typekits
...
class OpsLogger : public RTT::Service {
	public:
		...
		/**
		  ...
		  */
		void sendEventOp(atrias_msgs::rt_ops_event_<shared::RtAlloc>& event);

	private:
		// Port used to send events
		RTT::OutputPort<atrias_msgs::rt_ops_event_<shared::RtAlloc>> eventOut;

		// Port used for logging
		RTT::OutputPort<atrias_msgs::log_data_<shared::RtAlloc>> logOut;
